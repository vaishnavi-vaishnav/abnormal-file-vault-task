...
import { formatBytes, parseSizeInput } from '../utils/format';
...
export const FileList: React.FC = () => {
  ...
  const handleApplyFilters = () => {
    const parsedMin = parseSizeInput(sizeMin);
    const parsedMax = parseSizeInput(sizeMax);
    setAppliedFilters({
      search: search || undefined,
      file_type: fileTypeFilter || undefined,
      size_min: parsedMin !== undefined ? String(parsedMin) : undefined,
      size_max: parsedMax !== undefined ? String(parsedMax) : undefined,
    ...
    <div className="p-6">
      ...
      {summary && (
        <div className="mb-4 p-3 bg-gray-50 rounded text-sm text-gray-700 space-y-1">
          <div>
            Storage saved: <strong>{formatBytes(summary.savings_bytes)}</strong>
          ...
          <div className="text-gray-500">
            Logical: {formatBytes(summary.total_logical_bytes)} · Actual: {formatBytes(summary.total_actual_bytes)}
          </div>
        </div>
      )}
      {!files || files.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">
            {hasActiveFilters ? 'No matching files' : 'No files'}
          </h3>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-3 py-2 text-left text-sm font-medium text-gray-500">Size</th>
                ...
                <tr key={file.id}>
                  ...
                  <td className="px-3 py-3 text-sm text-gray-900">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      {file.stored_file && file.stored_file.ref_count > 1 && (
                        <span className="flex-shrink-0 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-800">
                          Deduped ×{file.stored_file.ref_count}
                        </span>
                      )}
                    </div>
                  </td>
        ...
        >
          ...
          >
            ...
            <div className="text-sm text-gray-500 space-y-1 mb-4">
              ...
              <div>Size: {formatBytes(detailsFile.size)}</div>
              <div>Uploaded: {new Date(detailsFile.uploaded_at).toLocaleString()}</div>
              {detailsFile.stored_file && detailsFile.stored_file.ref_count > 1 && (
                <div className="text-amber-700">
                  Shared with {detailsFile.stored_file.ref_count - 1} other upload{detailsFile.stored_file.ref_count > 2 ? 's' : ''} (deduplicated)
                </div>
              )}
            </div>
